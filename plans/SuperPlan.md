...
- [ ] Сохранить минимальную воспроизводящую CERN-задачу, ожидаемый вывод, C# output и классификацию остановки.
- [ ] Если различается поток команд, получить канонические C++ и C# traces одинакового запуска.
- [ ] Запустить `tools/diff_trace.py` и зафиксировать первую различающуюся PRE/POST строку, а не поздний симптом в stdout.
- [ ] Написать узкий падающий MSTest на найденную команду, экстракод или состояние загрузчика.
- [ ] Реализовать минимальное исправление без изменения эталонного input/expect.
- [ ] Запустить узкий тест, весь соответствующий класс, полную MSTest-регрессию и затронутый CERN batch.
- [ ] Коммитить каждую независимую семантическую причину отдельно.
