...
- [ ] Переписать `_count_cernlib.ps1`, чтобы корень репозитория вычислялся от `$PSScriptRoot`, а путь к эталону принимался параметром; удалить абсолютный `E:\Projects\...`.
- [ ] Добавить тест, который требует ровно 183 случая `lib1`, 214 случаев `lib2`, уникальность пары `(Library, Name)` и наличие `.f`/`expect_*.txt` для каждого активного случая.
- [ ] Запустить manifest-тест и подтвердить его падение при ещё отсутствующем `CernLibManifest`.
- [ ] Сгенерировать/зафиксировать manifest из активных вызовов `test_cernlib` в `ref/tests/cernlib_test.cpp`; закомментированные строки не включать.
- [ ] Заменить два `DataRow` на dynamic data из `CernLibManifest.ActiveCases`.
- [ ] Оставить `w303` отдельным `[Ignore]` с причиной и ссылкой на эталонное исключение.
- [ ] Выполнить:
...
**Критерий готовности:** одна команда выдаёт 183 + 214, тестовый адаптер видит 397 активных случаев, а несовпадение manifest с эталоном ломает тест.
...
- [ ] Тестом зафиксировать нормализацию только переводов строк; пробелы, пустые строки и управляющие символы остаются значимыми.
- [ ] Тестом зафиксировать отдельные каталоги артефактов для `lib1/name` и `lib2/name`, чтобы одинаковые имена не перезаписывались.
- [ ] Тестом зафиксировать восстановление `Console.Out` после исключения загрузчика.
- [ ] Добавить настраиваемые instruction и wall-clock limits; превышение должно давать отдельную классификацию, не общий output mismatch.
- [ ] Сохранять `actual`, unified diff, параметры запуска, instruction count и причину остановки для каждого отказа.
- [ ] Добавить фильтр/переменную окружения для детерминированного batch-разбиения всей матрицы, не меняя manifest.
- [ ] Запустить изолированные тесты фикстуры и два существующих beacon-теста.
